...
- `Mcp.Net.LLM`: `docs/roadmap/llm.md`
  - Current focus: finish the remaining LLM review follow-ons first; cancellation is deferred until the MCP client/tool path actually needs and supports it cleanly.
...
- The completed `Mcp.Net.LLM` metadata, option-cleanup, and tool-registration slices already touched `Mcp.Net.WebUi`; the remaining review follow-ons should stay local unless shared initialization or request contracts broaden.
