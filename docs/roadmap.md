...
## Current priorities
1. Revisit `IMcpClient` ergonomics in `Mcp.Net.Agent` now that the local file/process tool stack is in place
...
- `Mcp.Net.Agent`: `docs/roadmap/agent.md`
  - Current focus: revisit `IMcpClient` ergonomics around tool-call cancellation and async disposal.
